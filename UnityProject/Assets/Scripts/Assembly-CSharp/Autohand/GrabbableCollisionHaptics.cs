	...
	public class GrabbableCollisionHaptics : MonoBehaviour
	{
		...
		private void OnCollisionEnter(Collision collision)
		{
			if (canPlay && (int)collisionTriggers == ((int)collisionTriggers | (1 << collision.gameObject.layer)) && body != null && (collision.collider.attachedRigidbody == null || collision.collider.attachedRigidbody.mass > 1E-07f))
			{
				...
				grab.PlayHapticVibration(Mathf.Clamp(velocityDurationCurve.Evaluate(magnitude), 0f, maxDuration), velocityAmpCurve.Evaluate(magnitude * velocityAmp) * hapticAmp);
				if (playRoutine != null)
				{
					StopCoroutine(playRoutine);
				}
