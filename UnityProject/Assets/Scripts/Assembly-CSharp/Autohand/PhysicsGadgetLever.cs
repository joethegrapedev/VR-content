...
namespace Autohand
{
	public class PhysicsGadgetLever : PhysicsGadgetHingeAngleReader
	{
		...
		private bool FindSteps()
		{
			...
			{
				stepMarkers[stepCount - i - 1] = minimum + (float)i * step;
			}
			return true;
		}
		...
		public void SetSpring(int step)
		{
			...
			currStep = step;
			JointSpring spring = GetJoint().spring;
			spring.targetPosition = stepMarkers[step];
			GetJoint().spring = spring;
		}

		public void SetSpring(float stepRotation)
		{
			JointSpring spring = GetJoint().spring;
			spring.targetPosition = stepRotation;
			GetJoint().spring = spring;
		}

		private float FindCurrentStep()
		{
			float num = GetValue() * GetRange();
			for (int i = 0; i < stepCount; i++)
			{
		...
		{
			return (Mathf.Abs(minimum) + Mathf.Abs(maximum)) / 2f;
		}
